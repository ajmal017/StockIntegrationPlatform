...
<div class="Cart">
    ...
    <div class="Cart__product">
        <div class="Cart__productGrid Cart__productImg">
            <img class="img-fluid" src="{{$item->associatedModel->img}}" alt="">
        </div>
        ...
        <div class="Cart__productGrid Cart__productQuantity">
            <button style="padding:5px" class="btn m-0 btn-sm btn-info btn-minus" data-itemid="{{$item->id}}">-</button>
            <span  class="qty" data-itemid="{{$item->id}}">{{$item->quantity}}</span>
            <button style="padding:5px" class="btn m-0 btn-sm btn-info btn-plus" data-itemid="{{$item->id}}">+</button>
        </div>
...
<script>
    ...
    $('.btn-minus').click(function () {
        ...
        $.ajax({
            method: 'POST',
            url: '/product/cart_total/upload/'+itemid,
            data: {
                quantity:-1
            },
    ...
    $('.btn-delete').click(function () {
        ...
        if (r==true){
            $.ajax({
                method: 'POST',
                url: '/product/cart_total/delete/'+itemid,
                data: {},
                success: function (res) {
                    window.location.reload()
                },
